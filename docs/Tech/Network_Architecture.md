...
!!! warning "常见陷阱" 1. **不要混用浮点和定点**: 帧同步中一旦混入 float，必然不同步。 2. **不要忽视 RTT 变化**: 延迟抖动会导致预测失效，需动态调整。 3. **不要过度同步**: AOI + 增量更新是大规模必需品。
...
!!! warning "随机数陷阱" 1. **避免使用 `System.DateTime.Now.Ticks` 作为种子** — 每台机器时间不同。 2. **避免在多线程中共享 Random 实例** — 竞态条件导致序列不同。 3. **避免在 UI 逻辑中使用战斗随机数** — 消耗了序列会导致不同步。 4. **粒子特效、音效的随机可以独立** — 这些不影响逻辑状态。
...
!!! caution "血泪教训" 1. **不要低估工作量** — 联网化通常需要 60-100% 的额外开发时间。 2. **不要后期才考虑联网** — 从第一天就用联网思维设计架构。 3. **不要忽视测试** — 联网 Bug 比单机 Bug 难 10x 定位。 4. **不要跳过原型验证** — 先用最简网络层验证核心玩法手感。
