...
async function installWithSvn(skill) {
  ...
  const claudeSkillsDir = path.join(__dirname, '..', '..');
...
async function installSkill(skill) {
  ...
  try {
    ...
    // 1. Try SVN if available
    if (hasCommand('svn')) {
      console.log(`   ✓ SVN detected - attempting efficient folder download`);
      try {
        installMethod = 'SVN';
        skillDir = await installWithSvn(skill);
        installed = true;
      } catch (error) {
        console.log(`   ⚠ SVN download failed: ${error.message}`);
        console.log(`   ⚠ Falling back to other methods...`);
      }
    }

    // 2. Try Git if SVN failed or wasn't available
    if (!installed && hasCommand('git')) {
      ...
      try {
        ...
        console.log(`   ⚠ Git sparse checkout failed: ${error.message}`);
        console.log(`   ⚠ Falling back to simple download...`);
      }
    }

    // 3. Last resort: Download SKILL.md only
    if (!installed) {
      console.log(`   ⚠ Using fallback method - downloading SKILL.md only`);
