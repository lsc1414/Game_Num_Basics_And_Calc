---
sidebarTitle: "核心数值体系定义手册"
title: "核心数值体系定义手册"
---
> **摘要**：本文围绕「核心数值体系定义手册」提供核心内容与可落地方法。
...
为了防止“连续不暴击导致漏怪”或“连续暴击导致瞬秒 Boss”，暴击判定**必须**采用伪随机 (PRD)。
...
- **对于近战 (Melee) / 动作类:**
  ...
  - _处理前摇：_ 在塔防/肉鸽中，通常不建议做复杂的“卡肉”或“取消后摇”。直接整体加速动画即可。如果攻速极快（如 5.0 APS），建议简化动画为残影，避免模型鬼畜。

### 2.2 ⏳ 冷却时间 (Cooldown / CD)

- **公式：** `FinalCD = BaseCD * (1 - min(TotalCDR, Cap))`
- **上限 (Cap):** 建议锁定 CDR 上限为 **75%** (即 4 倍施法频率)。
  - _设计意图：_ 防止无限控制或服务器崩溃。
...
- _逻辑：_ 怪物进入圈 -> 塔开火 -> 子弹理论上一定能打到（除非怪跑得比子弹快飞出了界）。
...
- **👀 警戒范围 (Acquisition Range):** **索敌专用**。
  - _定义：_ 英雄在这个范围内发现敌人，并开始**移动**去追击。
  - _数值：_ 通常较大（如 800）。
- **🤺 攻击距离 (Weapon Range / Reach):** **开火专用**。
  - _定义：_ 英雄手中的刀或枪能打多远。
  - _数值：_ 近战通常很小（如 150），远程取决于武器（如 600）。
- _AI 状态机：_ 警戒范围发现怪 -> 移动靠近 -> 到达攻击距离 -> 停止移动并播放攻击动画。
...
- **CritMultiplier (暴击期望倍率):** `1 + (CritRate * (CritDamage - 1))`
  - _注：如果 CritRate > 100% (红色暴击)，公式依然通用。_
...
- _UI 变色：_ 如果当前 CD 小于 BaseCD，用 **绿色** 显示，表示受到增益。
...
- _示例：_ 1000 血，50% 减伤，50% 闪避。
- `EHP = 1000 / (0.5 * 0.5) = 4000`。
- _意义：_ 闪避和护甲乘算后，极大提升了有效血量。
...
以 **1 点 攻击力 = 1 CP** 为锚点。权重设计应反映该属性获取的“稀缺度”。
...
- _注意：_ 这样做会增加计算复杂度，且可能导致玩家换了一件装备，明明属性变了，战力却因为阈值波动而没变，造成困惑。**推荐前期只用静态权重求和。**
...
- _示例：_ 技能自带 10% 几率流血。玩家幸运+50%。最终触发率 = `10% * 1.5 = 15%`。
...
- **预填充 (Pre-fill):** 关卡开始时，将蓄水池初始化为 `Threshold * 0.8`。
  - _效果:_ 玩家只需击杀少量怪物，就能立刻触发第一次掉落。
- **首杀必掉 (First Blood Guarantee):**
  - _机制:_ 记录一个状态位 `HasDroppedFirstLoot`。
  - _逻辑:_ 若为 `false` 且击杀的是 **精英/BOSS**，无视蓄水池，强制掉落并置为 `true`。
...
- **HP 成长:** 推荐 `Factor = 1.2` (每分钟/每波)。
  - _第 10 分钟:_ `1.2 ^ 10 ≈ 6.19` 倍。
  - _第 30 分钟:_ `1.2 ^ 30 ≈ 237` 倍。
...
- **精英替换 (Elite Swap):**
  - _0-5min:_ 普通怪。
  - _5-10min:_ 替换为带盾/高甲怪 (Model Scale 1.2x)。
  - _10min+:_ 替换为小 Boss 模型 (Model Scale 1.5x)。
- **巨型单位 (Colossus):**
  - 当同屏怪数达到上限但压力仍不够时，生成 **Colossus**。
  - **1 Colossus = 20 Minions** 的数值强度。
  - _作用:_ 节省 DrawCall，增加压迫感。
...
- _注意：_ Boss 必须拥有高韧性，否则会被永久晕眩。

#### C. 状态易伤 (Damage to CC)

- **定义：** 独立乘区 (More)。
- **词条示例：** “对冰冻敌人伤害 +50%”、“对昏迷敌人伤害 +100%”。
- _价值：_ 鼓励玩家构建“控制+输出”的组合技。
...
- **公式 (定额模式 - Flat Block):**
  - _定义:_ 盾牌常驻属性，如“格挡 50 点伤害”。
  - _效果:_ `Damage = max(0, Damage - BlockValue)`。
  - _用途:_ 针对高频低伤怪物的神技，对 Boss 大招无效。
...
- **击中回复 (LoH):** 每次击中回复固定数值 (Flat)。
  - _适用:_ 高攻速、低单发伤害流派。
- **吸血 (Leech):** 回复伤害的百分比。
  - _塔防限制:_ 建议仅对英雄单位生效。如果塔也吸血，需要在塔上显示绿字，可能会导致 UI 混乱。建议塔使用“击杀回复 (Life on Kill)”。
...
- **继承系数:**
  - HP/攻击力: 通常继承 100%。
  - 暴击/攻速: 通常继承 100%。
  - _特殊:_ 某些词条专门加成召唤物（“召唤物伤害 +50%”），这是独立乘区。
...
- **稀释效应 (Dilution):**
  ...
  - _结论:_ 同样数值的词条，你拥有的越多，新获得的那个就越不值钱。
...
- _体验:_ 这是一种“软强制”的装备更新驱动力，比单纯加 HP 更有效。
