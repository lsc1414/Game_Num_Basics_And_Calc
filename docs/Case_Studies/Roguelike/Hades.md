---
sidebarTitle: "Hades 构建多样性深度解析：从设计哲学到底层架构"
title: "Hades 构建多样性深度解析：从设计哲学到底层架构"
---
> **摘要**：本文聚焦「Hades 构建多样性深度解析：从设计哲学到底层架构」，梳理核心概念、关键方法与落地实践。

---

**文档目标**：全方位解析 _Hades_ 如何通过“双层标签”设计与“模块化代码”架构，在有限资源下创造出 **225+** 种差异巨大的核心战斗体验（Build）。本文档分为**设计篇**与**程序架构篇**。
...
- **Ares**: `Doom` (延迟伤害)
- **Aphrodite**: `Weak` (虚弱)
- **Dionysus**: `Hangover` (中毒 DOT)

### 2.2 触发标签 (Trigger Tags) - 机制改变

有些祝福不是为了造成伤害，而是为了**响应**标签：

- _例如：“对 `Weak` 状态的敌人造成额外伤害”_
- _例如：“当你造成 `Knockback` 时，额外造成一次 `Rupture` (撕裂) 伤害”_
...
当玩家同时拥有神 A 和 神 B 的特定前置祝福时，会解锁 Duo Boon。这是 _Hades_ 构建深度的巅峰。
...
_总结：真正的多样性不是来自于元素的数量，而是来自于元素之间连接方式的数量。通过良好的底层架构，让策划人员能够像搭积木一样自由组合玩法，而无需每次都请求程序修改代码。_
